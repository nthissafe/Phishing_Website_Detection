...
url = 'https://raw.githubusercontent.com/nthissafe/Phishing_Website_Detection/main/5.urldata.csv'
s = requests.get(url).content
data0 = pd.read_csv(io.StringIO(s.decode('utf-8')))
data = data0.drop(['Domain','URL_Length','URL_Depth'], axis = 1).copy()
print('\nDownloading dataset with 17 features')
data = data.sample(frac=1).reset_index(drop=True)
data.head()

print('\nData preprocessing and spliting dataset')
y = data['Label']
...
loaded_model = XGBClassifier(learning_rate=0.4,max_depth=7)
...
def featureExtraction(url):
  print('\nTaking URL from user to detect')
  features = []
...
link1.grid(columnspan=5,pady=5)
link1.bind("<Button-1>", lambda e: callback("https://en.wikipedia.org/wiki/Phishing"))

link2 = Label(detailsframe, text="Connect with github", fg="blue", cursor="hand2",font=('default',12),bg='#d8dbe3',height=2)
link2.grid(columnspan=5,pady=5)
link2.bind("<Button-1>", lambda e: callback("https://github.com/nthissafe/Phishing_Website_Detection"))
